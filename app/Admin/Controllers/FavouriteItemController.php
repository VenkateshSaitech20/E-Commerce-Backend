...
class FavouriteItemController extends AdminController
{
     ...
     * @var string
     */
    protected $title = 'Favourite Products';
    ...
    protected function grid()
    {
        ...
        $grid->column('customer_id', __('Buyer Name'))->display(function($customer_id){
            $customer_name = Customer::where('id',$customer_id)->value('customer_name');
            return $customer_name;
        });
        $grid->column('restaurant_id', __('Seller'))->display(function($restaurant_id){
        ...
        $items = Item::pluck('item_name', 'id');
            ...
            $filter->equal('customer_id', __('Buyer'))->select($customers);
            $filter->equal('restaurant_id', __('Seller'))->select($restaurants);
            $filter->equal('item_id', __('Item'))->select($items);
    ...
    protected function form()
    {
        ...
        $form->select('customer_id', __('Buyer Name'))->options($customers)->rules(function ($form) {
            return 'required';
        });
        $form->select('restaurant_id', __('Seller'))->options($restaurants)->rules(function ($form) {
            return 'required';
        });
