...
class ItemController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('restaurant_id', __('Seller'))->display(function($res_id){
        ...
        $grid->column('preparation_time', __('Turn Around Time'));
        ...
        if(!Admin::user()->isAdministrator()){
            ...
            $filter->equal('restaurant_id', __('Seller'))->select($restaurants);
            $filter->equal('in_stock', __('In stock'))->select(['0' => 'No', '1'=> 'yes'])->default('m');
            $filter->equal('is_approved', __('Is Approved'))->select(['0' => 'No', '1'=> 'Yes'])->default('m');
            $filter->like('item_name', __('Item Name'));
        }
    ...
    protected function form()
    {
        ...
        $form->tab('Item Details', function ($form) {
            ...
            if(!Admin::user()->isAdministrator()){
                ...
                $form->select('restaurant_id', __('Seller'))->options($restaurants)->rules('required');
            ...
            $form->decimal('preparation_time', __('Turn Around Time'))->placeholder('Input Minutes...')->rules('required');
            $form->select('is_recommand_tag', __('Is Recommended'))->options(['0' => 'No', '1'=> 'Yes'])->default('1')->rules('required');
        ...
        })->tab('Item Catalogue', function ($form) {
             $form->hasMany('item_catalogues', function (Form\NestedForm $form) {
                $form->select('file_type', __('File Type'))->options(['1' => 'Image', '2'=> 'Video'])->default('1')->rules('required');
                $form->image('image', __('Image'))->uniqueName()->move('catalogues/');
                $form->text('video', __('Video'))->rules(function ($form) {
                    return 'required';
                });
            });
        });
